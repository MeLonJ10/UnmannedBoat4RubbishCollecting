# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 21:29:09 2019
...
class darknet_tiny(object):
    ...
    def forward(self, inputs):
        ...
        inputs = common._conv2d_fixed_padding(inputs, 32//4,  3, strides=1)
        inputs = common._conv2d_fixed_padding(inputs, 64//4,  3, strides=2)
        inputs = self._darknet_block(inputs, 32//4)
        inputs = common._conv2d_fixed_padding(inputs, 128//4, 3, strides=2)

#        for i in range(2):
        for i in range(1):
            inputs = self._darknet_block(inputs, 64//4)

        inputs = common._conv2d_fixed_padding(inputs, 256//4, 3, strides=2)

#        for i in range(8):
        for i in range(4):
            inputs = self._darknet_block(inputs, 128//4)

        route_1 = inputs
        inputs = common._conv2d_fixed_padding(inputs, 512//4, 3, strides=2)

#        for i in range(8):
        for i in range(4):
            inputs = self._darknet_block(inputs, 256//4)

        route_2 = inputs
        inputs = common._conv2d_fixed_padding(inputs, 1024//4, 3, strides=2)

#        for i in range(4):
        for i in range(2):
            inputs = self._darknet_block(inputs, 512//4)
